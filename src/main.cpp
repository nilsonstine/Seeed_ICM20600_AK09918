...
#define I2CDEV_IMPLEMENTATION == I2CDEV_TEENSY_4X_WIRE
...
uint8_t getDeviceID() {
    ...
    Wire1.requestFrom(0x69, (uint8_t)1);
    if (Wire1.available()) {
        return Wire1.read();
    } else {
        return 0;
...
void setup() {
    ...
    Wire1.begin();
    Serial.println("ABO ID: " + String(getDeviceID()));
    ...
    // uint8_t _buffer[16];
    // Wire1.read(_addr, ICM20600_WHO_AM_I, _buffer);
    // Serial.println("Device ID perhaps?:" + _buffer[0]);
